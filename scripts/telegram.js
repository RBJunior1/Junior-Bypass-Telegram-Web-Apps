...
(function() {
  ...
  // Function to change the src of iframes and set the user agent
  function modifyIframe() {
      // Find all iframe tags
      document.querySelectorAll('iframe').forEach(iframe => {
          // Check if the iframe's src contains 'tgWebAppPlatform=web'
  ...
  // Run initially to modify existing iframes
  modifyIframe();

  // Function to temporarily change the title
  function changeTitleTemporarily() {
      let originalTitle = document.title;
      document.title = "Telegram has been bypassed";
      setTimeout(() => {
          document.title = originalTitle;
      }, 10000);
  }

  // Call the function to temporarily change the title
  changeTitleTemporarily();
